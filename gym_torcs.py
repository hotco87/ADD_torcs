...
class TorcsEnv:
    ...
    def step(self, u):
       #print("Step")
        ...
        # track = np.array(obs['track'])
        # sp = np.array(obs['speedX'])
        # progress = sp*np.cos(obs['angle']) + sp*2 #np.cos(obs['angle'])*2
        # reward = progress

        # reward 2
        # track = np.array(obs['track'])
        # trackpos = np.array(np.abs(obs['trackPos']))
        # sp = np.array(obs['speedX']/8.0)
        # progress = sp * np.cos(obs['angle']) - trackpos * 30
        # reward = progress
        #
        # print("trackpos: ",trackpos*10)
        # print("reward: ", reward)
